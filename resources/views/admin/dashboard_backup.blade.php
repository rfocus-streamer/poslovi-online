                                    ...
                                    <tr>
                                        ...
                                        <td>
                                            <a href="?tab=users&users_search={{ urlencode($transaction->user->email) }}"
                                               class="text-decoration-none d-flex align-items-center gap-2"
                                               title="Prikaži korisnika">
                                               <img src="{{ $transaction->user->avatar ? Storage::url('user/' . $transaction->user->avatar) : asset('images/default-avatar.png') }}"
                                                     alt="Avatar" width="30" height="30" class="rounded-circle">
...
document.addEventListener('DOMContentLoaded', function() {
    // Provera da li u URL-u postoji users_search parametar
    const urlParams = new URLSearchParams(window.location.search);
    const usersSearch = urlParams.get('users_search');
    const tab = urlParams.get('tab');

    // Ako postoji users_search parametar, otvorite Users tab
    if (usersSearch && tab === 'users') {
        const triggerEl = document.querySelector('a[href="#users"]');
        if (triggerEl) {
            new bootstrap.Tab(triggerEl).show();
        }
    }

    // Fokus na pretragu u Users tabu
    if (tab === 'users') {
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    document.querySelectorAll('[data-bs-target="#transactionDetailsModal"]').forEach(btn => {
        btn.addEventListener('click', function() {
            ...
            // AJAX poziv za dohvatanje detalja
